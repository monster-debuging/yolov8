...
#     dist_x = torch.abs(cx1 - cx2)
#     dist_y = torch.abs(cy1 - cy2)
    
#     # 5. SIoU 角度惩罚 ← 修复 asin 范围
#     diagonal = torch.sqrt(dist_x ** 2 + dist_y ** 2) + eps
#     sin_alpha = dist_y / diagonal
#     sin_alpha = torch.clamp(sin_alpha, -1.0, 1.0)  # ← 关键修复！
#     angle = 2 * torch.asin(sin_alpha) - torch.pi / 2
#     angle_loss = torch.cos(angle)
    
#     # 6. SIoU 距离惩罚 ← 修复除零
#     diag = cw ** 2 + ch ** 2 + eps
#     dist_loss = (dist_x ** 2 + dist_y ** 2) / torch.clamp(diag, min=eps)
    
#     # 7. 形状惩罚 ← 修复数值爆炸
#     w1, h1 = b1_x2 - b1_x1, b1_y2 - b1_y1
#     w2, h2 = b2_x2 - b2_x1, b2_y2 - b2_y1
#     shape_w = 1 - torch.exp(-torch.clamp(torch.abs(w1 - w2) / (cw + eps), max=10))
#     shape_h = 1 - torch.exp(-torch.clamp(torch.abs(h1 - h2) / (ch + eps), max=10))
#     shape_loss = shape_w + shape_h
    
#     # 8. 总损失 ← 降低权重，更稳定
#     loss = 1 - iou + angle_loss * 0.25 + dist_loss * 0.25 + shape_loss * 0.125
...
#             print("Warning: pred_bboxes contains NaN!")
#         if torch.isnan(target_bboxes).any():
#             print("Warning: target_bboxes contains NaN!")

#         # 计算 SIoU
#         loss_iou = SIoU(pred_bboxes[fg_mask], target_bboxes[fg_mask])

#         # 【调试】检查输出是否有 NaN
#         if torch.isnan(loss_iou).any():
#             print("Warning: loss_iou contains NaN!")
#             print("loss_iou stats:", loss_iou.min(), loss_iou.max(), loss_iou.mean())
